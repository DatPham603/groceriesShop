...
export class OrderComponent implements OnInit{
  ...
  constructor(
    ...
    this.orderForm = this.formBuilder.group({
      full_name: ['đạt xx', Validators.required], // fullname là FormControl bắt buộc      
      email: ['dat@gmail.com', [Validators.email]], // Sử dụng Validators.email cho kiểm tra định dạng email
  ...
  placeOrder() {
    ...
    if (this.orderForm.valid) {
      ...
      this.orderService.placeOrder(this.orderData).subscribe({
        ...
        error: (error: any) => {
          debugger;
          alert(`Lỗi khi đặt hàng: ${error.error}`);
        },
      });
    } else {
